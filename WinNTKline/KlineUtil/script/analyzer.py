...
def is_text_file(file_path):
    """判断是否为文本文件"""
    text_exts = {'.py', '.txt', '.md', '.java', '.c', '.cpp', '.h', '.hpp',
                 '.cs', '.go', '.rs', '.swift', '.kt', '.js', '.ts', '.html',
                 '.css', '.scss', '.less', '.json', '.yml', '.yaml', '.xml'}
...
def print_results(total, details, use_color=True):
    ...
    print(f"{color_map['magenta']}▏ 总重复行数: {total['dup']} "
...
def main():
    ...
    try:
        ...
        if os.path.isfile(args.source) and os.path.isfile(args.target):
            result = file_compare(args.source, args.target, os.path.dirname(args.source), args.diff)
            ...
            print(f"{Fore.MAGENTA if use_color else ''}▏ 重复行数: {result['dup']} "
